...
export class CompraService {
  ...
  async create(asocaidoId: string, dto: CompraDto) {
  ...
  async createOne(asocaidoId: string, dto: CompraHechoDto) {
    const connection = getConnection();
    const queryRunner = connection.createQueryRunner();
    await queryRunner.connect();
    ...
    await queryRunner.startTransaction();
    try {
      const compra = this._compraEncabezadoRepository.create({
      ...
      for (let index = 0; index < dto.compraDetalles.length; index++) {
        ...
        insumo.insumoId = insumoDto.insumoId;
        insumo.cantidad = insumoDto.cantidad;
        insumo.precio = insumoDto.precio;
        insumo.total = insumoDto.total;
