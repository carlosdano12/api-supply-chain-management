...
export class compra1616284662516 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'CREATE TABLE `compras_encabezado` (`id` varchar(36) NOT NULL, `asociadoIdAsociado` varchar(255) NOT NULL, `nota` varchar(45) NOT NULL, `fechaSolicitud` date NOT NULL DEFAULT now(), `fechaCompra` date NOT NULL DEFAULT now(), `estado` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (`id`)) ENGINE=InnoDB',
    );
    await queryRunner.query(
      'CREATE TABLE `compra_detalle` (`id` varchar(36) NOT NULL, `compraEncabezadoId` varchar(255) NOT NULL, `insumoId` varchar(255) NOT NULL, `cantidad` decimal(13,2) NULL, `estado` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (`id`)) ENGINE=InnoDB',
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `compras_encabezado` ADD CONSTRAINT `FK_34c7da8bf2cd44e2a029aa13d87` FOREIGN KEY (`asociadoIdAsociado`) REFERENCES `asociados`(`id_asociado`) ON DELETE NO ACTION ON UPDATE NO ACTION',
    );
    await queryRunner.query(
      'ALTER TABLE `compra_detalle` ADD CONSTRAINT `FK_288f91b1d344e593aef0c1323a5` FOREIGN KEY (`compraEncabezadoId`) REFERENCES `compras_encabezado`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION',
    );
    await queryRunner.query(
      'ALTER TABLE `compra_detalle` ADD CONSTRAINT `FK_0bab75ce3a4c06ce096cc9edcba` FOREIGN KEY (`insumoId`) REFERENCES `insumos`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION',
    );
  }
