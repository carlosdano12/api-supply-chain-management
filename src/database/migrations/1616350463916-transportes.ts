...
export class transportes1616350463916 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'CREATE TABLE `transporte` (`id` varchar(36) NOT NULL, `asociadoIdAsociado` varchar(255) NOT NULL, `nota` varchar(45) NOT NULL, `fechaSolicitud` date NOT NULL DEFAULT now(), `fechaEntrega` date NOT NULL DEFAULT now(), `estado` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (`id`)) ENGINE=InnoDB',
    );
    await queryRunner.query(
      'CREATE TABLE `transporte_detalle` (`id` varchar(36) NOT NULL, `transporteId` varchar(255) NOT NULL, `niameIdNiame` varchar(255) NOT NULL, `cantidad` decimal(13,2) NULL, `estado` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (`id`)) ENGINE=InnoDB',
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `transporte` ADD CONSTRAINT `FK_e2aa53f2854f78a70e12c7a4c32` FOREIGN KEY (`asociadoIdAsociado`) REFERENCES `asociados`(`id_asociado`) ON DELETE NO ACTION ON UPDATE NO ACTION',
    );
    await queryRunner.query(
      'ALTER TABLE `transporte_detalle` ADD CONSTRAINT `FK_8a461c87c0974a4d7389b5d06e3` FOREIGN KEY (`transporteId`) REFERENCES `transporte`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION',
    );
    await queryRunner.query(
      'ALTER TABLE `transporte_detalle` ADD CONSTRAINT `FK_e1e3c36d01cf1b61aa6b1dc0ff6` FOREIGN KEY (`niameIdNiame`) REFERENCES `niames`(`id_niame`) ON DELETE NO ACTION ON UPDATE NO ACTION',
    );
  }
