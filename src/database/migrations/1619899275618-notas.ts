...
export class notas1619899275618 implements MigrationInterface {
  name = 'notas1619899275618';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `ventas_encabezado` CHANGE `fechaVenta` `fechaVenta` date NOT NULL DEFAULT now()',
    );
    ...
    await queryRunner.query('ALTER TABLE `venta_detalle` CHANGE `niameIdNiame` `niameIdNiame` varchar(36) NULL');
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('ALTER TABLE `venta_detalle` DROP FOREIGN KEY `FK_de46914ffeb7efdbb333cecc772`');
    await queryRunner.query(
      "ALTER TABLE `venta_detalle` CHANGE `niameIdNiame` `niameIdNiame` varchar(36) NULL DEFAULT 'NULL'",
    );
    await queryRunner.query(
      "ALTER TABLE `venta_detalle` CHANGE `cantidad` `cantidad` decimal(13,2) NULL DEFAULT 'NULL'",
    );
    await queryRunner.query(
      'ALTER TABLE `venta_detalle` ADD CONSTRAINT `FK_de46914ffeb7efdbb333cecc772` FOREIGN KEY (`niameIdNiame`) REFERENCES `niames`(`id_niame`) ON DELETE NO ACTION ON UPDATE NO ACTION',
    );
    await queryRunner.query(
      'ALTER TABLE `ventas_encabezado` CHANGE `fechaVenta` `fechaVenta` date NOT NULL DEFAULT CURRENT_TIMESTAMP()',
    );
    await queryRunner.query('ALTER TABLE `ventas_encabezado` CHANGE `nota` `nota` varchar(45) NOT NULL');
    await queryRunner.query(
    ...
    await queryRunner.query('ALTER TABLE `transporte` CHANGE `nota` `nota` varchar(45) NOT NULL');
    await queryRunner.query(
    ...
    await queryRunner.query('ALTER TABLE `compras_encabezado` CHANGE `nota` `nota` varchar(45) NOT NULL');
  }
}
