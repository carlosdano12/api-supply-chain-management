...
export class ventas1619898887301 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("CREATE TABLE `ventas_encabezado` (`id` varchar(36) NOT NULL, `asociadoIdAsociado` varchar(255) NOT NULL, `nota` varchar(45) NOT NULL, `fechaVenta` date NOT NULL DEFAULT now(), `estado` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (`id`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `venta_detalle` (`id` varchar(36) NOT NULL, `ventaEncabezadoId` varchar(255) NOT NULL, `cantidad` decimal(13,2) NULL, `estado` tinyint NOT NULL DEFAULT 0, `niameIdNiame` varchar(36) NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB");
        ...
        await queryRunner.query("ALTER TABLE `ventas_encabezado` ADD CONSTRAINT `FK_d5706c1446500eeb2972d341361` FOREIGN KEY (`asociadoIdAsociado`) REFERENCES `asociados`(`id_asociado`) ON DELETE NO ACTION ON UPDATE NO ACTION");
        await queryRunner.query("ALTER TABLE `venta_detalle` ADD CONSTRAINT `FK_6dd731142d80693853761b9562b` FOREIGN KEY (`ventaEncabezadoId`) REFERENCES `ventas_encabezado`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION");
